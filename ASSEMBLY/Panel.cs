...
public class Panel : IActionListener, IChatable
{
	...
	private void paintBottomMoneyInfo(mGraphics g)
	{
		if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
		{
			...
			g.drawImage(imgLuong, X + 90, H - 8, 3);
			mFont.tahoma_7_yellow.drawString(g, Char.myCharz().xu + string.Empty, X + 24, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			mFont.tahoma_7_yellow.drawString(g, Char.myCharz().luong + string.Empty, X + 100, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			g.drawImage(imgLuongKhoa, X + 140, H - 8, 3);
			mFont.tahoma_7_yellow.drawString(g, Char.myCharz().luongKhoa + string.Empty, X + 150, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
		}
	}
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 8004)
		{
			...
			GameScr.TeleVip(infoItem3.charInfo.charID);
		}
		if (idAction == 8005)
		{
			GameScr.TeleVip((int)p);
		}
		if (idAction == 8001)
		{
